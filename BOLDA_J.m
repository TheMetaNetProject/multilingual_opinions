...
function [ WP , DP , OP, Z_O, Z_W, phi_W, phi_O1, phi_O2, theta, ENTRY_SERIES, entrymap] = BOLDA_J(x_w, d_w, x_o, d_o, l_o, l_w, alpha, beta_W, beta_O, gamma, T, EM_iters, pi, matchmode, savefilename, optimize, matchesonly)
    pi = ceil(pi);
...
%    EM_iters = 3;
    ...
    if matchesonly==0
        ...
        for m = 1:total_M
            if (dictionary(m,1)>0)&&dictionary(m,2)>0
                wi = dictionary(m,1);
                numentriesvector(wi) = numentriesvector(wi)+1;
                wi = dictionary(m,2)+ Voc_W1;
                numentriesvector(wi) = numentriesvector(wi)+1;
    ...
    save(savefilename)
    [ WP , DP , Z_W, OP , Z_O, ENTRY_SERIES ] = GibbsSamplerBOLDA_J(x_w, d_w, T, Gibbs_iters, alpha, beta_W, l_o, x_o, d_o, numentriesvector, beta_O , gamma, seed, OUTPUT, entryvector, matchesonly);
    save(savefilename)
