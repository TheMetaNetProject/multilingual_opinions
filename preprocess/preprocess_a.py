'''
Created on 17 Jan 2013

Last Modified: 29 Jan 2013

@author: E.D. Gutierrez email: edg@icsi.berkeley.edu
'''
opinionwords =  '' #'verbsonly' #'adjectives' #'' 'verbsonly'
w_text = ''
o_text = ''
lang = 'ru'
basedir = '/u/metanet/Parsing/parsedblogs_'+lang+'/'
o_outfilename = basedir+'o_'+lang+'_nocomments'
w_outfilename = basedir+'w_'+lang+'_nocomments.txt'
if opinionwords=='verbsonly':
    o_outfilename = o_outfilename + '_' + opinionwords + '.txt'
else:
    if opinionwords=='adjectives':
        o_outfilename = o_outfilename + '_' + opinionwords +'.txt'
    else:
        o_outfilename = o_outfilename + '.txt'

for ii in range(1,18):
    number = str(ii)
    infilename = basedir+number+'.lem' #matized'
    infile = open(infilename)
    documentcounter = 1
    o_counter = 1
    w_counter = 1
    if lang=='es':
        wtag_list = ['nc']
        if opinionwords=='verbsonly':
             otag_list = ['vm']
        else:
            if opinionwords=='adjectives':
                otag_list = ['aq','rg']
            else:
                otag_list = ['vm', 'aq', 'rg']
        discardtag_list = ['p0', 'np','pr','pp','da','sp', 'rn', 'di','fc','fp', 'cs','cc', 'vs','pi','va','fx','z']
    else:
        if lang=='en':
            wtag_list = ['nn', 'nns']
            if opinionwords=='verbsonly':
                otag_list = ['vb', 'vbd', 'vbg', 'vbn', 'vbp', 'vbz']
            else:
                if opinionwords=='adjectives':
                    otag_list = ['jj', 'jjs', 'jjr']
                else:
                    otag_list = ['jj', 'jjs', 'vb', 'vbd', 'vbg', 'vbn', 'vbp', 'vbz','jjr']
            discardtag_list = ['rb', 'rbr','rbs','in', 'nnp', 'nnps', 'dt', '_:', 'ls', 'fw', '_``','wrb',  'rp','ban_torture-subscribe@yahaoogroups.com_nn', 'sym', 'prp', 'prp$', '$', 'moh\/_nn', 'pdt', 'to', 'md', 'wp$', 'wdt', '-lrb-', 'pos','cc', 'pos' ',', '.', '``', '\'\'', 'uh','cd', '-rrb-',',',':','_\'\'', 'ex', 'wp', '_nnp','__nnp','___nnp','____nnp','_____nnp','_________nnp', '________nnp','_nn','__nn','___nn','____nn','_____nn','___cd','_____rb','_______nnp','____rb','__fw','____fw','pub_nn']
        else:
            wtag_list = ['no', 'noun...']
            discardtag_list = ['']
            if opinionwords=='verbsonly':
                otag_list = ['ve', 'verb...']
            else:
                if opinionwords=='adjectives':
                    otag_list = ['ad', 'adjective...']
  
                else:
                    otag_list = ['ve', 'verb...','ad', 'adjective...']

    for line in infile:
        words = str.lower(line)
        words = words.split()
        for word in words:
            try:
                tag = word[(word.find('_')+1):(word.find('_')+3)]
                word = word[:word.find('_')]
            except:
                tag = word[(word.find('_')+1):(word.find('_')+2)]
                word = word[:word.find('_')]
            try:
                wtag_list.index(tag)
                w_text = w_text+word + ' '          
            except:
                try:
                    otag_list.index(tag)
                    o_text = o_text + word + ' '
                except:
                    try:
                        discardtag_list.index(tag)
                    except:
                        discardtag_list.append(tag)
                        print tag+' '+number
        w_text = w_text + '\n'
        o_text = o_text + '\n'
outfile1 = open(o_outfilename,'w')
outfile1.write(o_text)
outfile1.close()
outfile2 = open(w_outfilename,'w')
outfile2.write(w_text)
outfile2.close()