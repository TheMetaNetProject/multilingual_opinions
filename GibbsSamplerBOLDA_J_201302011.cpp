...
// NEW VARIABLES: c, m, L
// Syntax
//   [ WP , DP , Z_W, OP , Z_O ] = GibbsSamplerBOLDA( WS , DS , T , N , ALPHA , BETA_W , L_O , MM , OS , ODS , C_NEW , BETA_O , GAMMA , SEED , OUTPUT )

// Syntax
//   [ WP , DP , Z_W, OP , Z_O ] = GibbsSamplerBOLDA( WS , DS , T , N , ALPHA , BETA_W , L_O , MM , OS , ODS , C_NEW , BETA_O , GAMMA , SEED , OUTPUT , Z_W_IN, Z_O_IN )
...
//   GibbsSamplerLDA(        ALPHA,        BETA_W,        BETA_O,        GAMMA,     Voc_W,     Voc_O1,     Voc_O2,     MaxMatches,     C,     T,     D,     NN,     OUTPUT,     n_W,     n_O,      z_W,      z_O,      d_W,      d_O,      w_W,      w_O,      wp,      dp,      op,      l_O,      d1,      numEntriesPerWord,      entrySeries,      ztot_W,      ztot_O1,      ztot_O2,      order_W,      order_O,         probs,     startcond );
void GibbsSamplerLDA( double ALPHA, double BETA_W, double BETA_O, double GAMMA, int Voc_W, int Voc_O1, int Voc_O2, int MaxMatches, int C, int T, int D, int NN, int OUTPUT, int n_W, int n_O, int *z_W, int *z_O, int *d_W, int *d_O, int *w_W, int *w_O, int *wp, int *dp, int *op, int *l_O, int *d1, int *numEntriesPerWord, int *entrySeries, int *ztot_W, int *ztot_O1, int *ztot_O2, int *order_W, int *order_O, double *probs, int startcond )
{
...
//\/\/\/\***SAMPLE***/\/\/\/\\  
  ...
  for (iter=0; iter<NN; iter++) { //NN is total number of iterations
      if (OUTPUT >=1) {
          if ((iter % 1)==0) mexPrintf( "\tIteration %d_W of %d_W\n" , iter , NN );
          if ((iter % 1)==0) mexEvalString("drawnow;");
      }
      for (ii = 0; ii < n_W; ii++) {
//mexPrintf("\tIter=%d, i = %d\n", iter, i);
          ...
          for (j = 0; j < T; j++) {  //For each topic, calculate the proportional probability
			  for (h = 0; h < ni_W; h++){
				  g++;
                  probs[g] = ((double) wp[ d1[MaxMatches*wi_W+h]*T + j ] + (double) BETA_W)/( (double) ztot_W[j]+ (double) WBETA)*( (double) dp[ dioffset+ j ] + (double) ALPHA);
                  totprob += probs[g];
			  }
          }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs,
                 const mxArray *prhs[])
{
  double *srwp, *srop, *srdp, *probs, *Z_W, *Z_O, *EntrySeries, *WS, *DS, *OS, *ODS, *L_O, *Z_W_IN, *Z_O_IN, *EntrySeries_IN, *C_new, *D1, *NumEntriesPerWord; 
  double ALPHA,BETA_W,BETA_O, GAMMA;
  mwIndex *irwp, *irop, *jcwp, *jcop, *irdp, *jcdp;
  int *z_W, *z_O, *d_W, *d_O, *w_W, *w_O, *l_O, *c_new, *order_W, *order_O, *wp, *op, *dp, *ztot_W, *ztot_O1, *ztot_O2, *d1, *numEntriesPerWord, *entrySeries;
  int MaxMatches, C, Voc_W, Voc_O1, Voc_O2,T,D,NN,SEED,OUTPUT, nzmax, nzmaxwp, nzmaxop, nzmaxdp, ntokens_W, ntokens_O, nentries_W;
  int i,j,c,n,n_W, n_O,nt,wi_W, wi_O, di_W, di_O, startcond;
...
// get the number of opinion tokens
  nentries_W = (int) mxGetM( prhs[ 14 ] ) * mxGetN( prhs[ 14 ] );
  ...
  D1 = mxGetPr( prhs[ 14] );
  NumEntriesPerWord = mxGetPr( prhs[ 9 ] );
 
  //pointer to language indices
  L_O = mxGetPr( prhs[ 6 ] );

  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  //if (M != mxGetM( prhs[ 14 ] ) * mxGetN( prhs[ 14 ] )) mexErrMsgTxt("Number of dictionary entries not the same in D1 and D2");
  ...
  GAMMA = (double) mxGetScalar(prhs[11]);
...
/* allocate memory */
  ...
  d1 = (int *) mxCalloc(nentries_W, sizeof( int ));
  ...
  for (i=0; i<nentries_W; i++) d1[ i ] = (int) D1[ i ] - 1;
 ...
 for (i=0; i<nentries_W; i++) {
     if (d1[ i ] > C) C = d1[ i ];  //get the total number of dictionary entries
  } 
mexPrintf("\tC=%d\n", C);
...
mexPrintf("\tMaxMatches=%d\n", MaxMatches);
  ...
  if (OUTPUT==2) {
      ...
      mexPrintf( "Arguments:\n" );
      mexPrintf( "\tNumber of words      Voc_W = %d_W\n"    , Voc_W );
      mexPrintf( "\tNumber of docs       D = %d_W\n"    , D );
      mexPrintf( "\tNumber of topics     T = %d_W\n"    , T );
      mexPrintf( "\tNumber of iterations N = %d_W\n"    , NN );
      mexPrintf( "\tHyperparameter   ALPHA = %4.4f\n" , ALPHA );
      mexPrintf( "\tHyperparameter    BETA_W = %4.4f\n" , BETA_W );
      mexPrintf( "\tHyperparameter    BETA_W = %4.4f\n" , BETA_O );
	  mexPrintf( "\tHyperparameter    BETA_W = %4.4f\n" , GAMMA );
	  mexPrintf( "\tSeed number            = %d_W\n"    , SEED );
      mexPrintf( "\tNumber of tokens -topic= %d_W\n"    , ntokens_W );
	  mexPrintf( "\tNumber of tokens -opin = %d_W\n"    , ntokens_O );
