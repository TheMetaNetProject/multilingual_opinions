function [ WP , DP , OP, Z_O, Z_W, phi_W, phi_O1, phi_O2, rho_1, rho_2, theta,mm, cost] = BOLDA(x_w, d_w, x_o, d_o, l_o, l_w, alpha, beta_W, beta_O, gamma, T, EM_iters, pi, matchmode, savefilename)
  %  pi = pi(2:end, 2:end);    
  ...
  %  pi = zeros(Voc_W1M, Voc_W2M);
    ...
    save(savefilename)    
    [Matching,cost] = lapjv(-pi(M1_indices, M2_indices-Voc_W1)); % HUNGARIAN LOOKS FOR MINIMUM EDGE WE WANT MAX EDGE
     for i = 1:Voc_W1M
        for j = 1:Voc_W2M
            if Matching(i)==j
...
function [mm, cost, mu] = matcher(pi, WP, beta_W, gamma,T, M1_indices, M2_indices, Voc_W1M, Voc_W2M, iter, x_w, l_w, Voc_W1)
    ...
    mu = zeros(max(M1_indices), max(M2_indices));
...
k
    ...
    for i = 1:Voc_W1M
        for j = 1:Voc_W2M
            WP1 = WP(M1_indices(i),:);
            WP2 = WP(M2_indices(j),:);
            mu(M1_indices(i),M2_indices(j)) = pi(M1_indices(i),M2_indices(j)) + sum(log(reshape((WP1 + WP2 + beta_W)./normalizer,size(WP(1,:)))).*(WP1+WP2)) - sum(WP1)*log(rho_1(i)) - sum(WP2)*log(rho_2(j));
