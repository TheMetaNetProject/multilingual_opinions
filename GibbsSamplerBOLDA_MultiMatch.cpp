...
// Adapted from code by Mark Stevers
// NEW VARIABLES: c, m, L
// Syntax
//   [ WP , DP , Z_W, OP , Z_O , EP] = GibbsSamplerBOLDA_MultiMatch( WS , DS , T , N , ALPHA , BETA_W , L_O , MM , OS , ODS , NUMENTRIESVEC, BETA_O  , SEED , OUTPUT, ENTRYVEC, RELEGATE )

// Syntax
//   [ WP , DP , Z_W, OP , Z_O, EP ] = GibbsSamplerBOLDA_MultiMatch( WS , DS , T , N , ALPHA , BETA_W , L_O , MM , OS , ODS , NUMENTRIESVEC, BETA_O , SEED , OUTPUT , ENTRYVEC, RELEGATE, Z_W_IN, Z_O_IN, ENTRYSERIES)
...
//   GibbsSamplerLDA(        ALPHA,        BETA_W,        BETA_O,     Voc_W,     Voc_O1,     Voc_O2,     MaxMatches,     C,     T,     D,     NN,     OUTPUT,     n_W,     n_O,      z_W,      z_O,      d_W,      d_O,      w_W,      w_O,      ep,     wp,    dp,      op,      l_O,      entryVec,      numEntriesPerWord,      entrySeries,      ztot_W,      ztot_O1,      ztot_O2,      order_W,      order_O,         probs,     startcond, int relegate );
void GibbsSamplerBOLDA_MultiMatch( double ALPHA, double BETA_W, double BETA_O, int Voc_W, int Voc_O1, int Voc_O2, int MaxMatches, int C, int T, int D, int NN, int OUTPUT, int n_W, int n_O, int *z_W, int *z_O, int *d_W, int *d_O, int *w_W, int *w_O, int *ep, int *wp, int *dp, int *op, int *l_O, int *entryVec, int *numEntriesPerWord, int *entrySeries, int *ztot_W, int *ztot_O1, int *ztot_O2, int *order_W, int *order_O, double *probs, int startcond, int relegate )
{
  ...
  { //START FROM INITIAL CONDITIONS
	  ...
	  if (relegate==0) {
		  for (i=0; i<n_W; i++)
		  {
			  ...
			  ep[ entry*T + topic ]++; // increment dp count matrix
		  }
	  } else {
		  for (i=0; i<n_W; i++)
		  {
			  ...
			  if (ni_W>0) {
                           ...
                           dp[ di_W*T + topic ]++; // increment dp count matrix
                           ztot_W[ topic ]++; // increment ztot_W matrix
                           ep[ entry*T + topic ]++; // increment ep count matrix
			  }
		  }
	  }
	  for (i=0; i<n_O; i++)
      ...
      for (i=0; i<n_W; i++)
      {
         ...
         // pick a random entry 0...ni_W - 1
          if (ni_W>0) {
              ...
              entry = (int) entryVec[ MaxMatches*wi_W + entry];
              entrySeries[ i ] = entry;
...
//\/\/\/\***SAMPLE***/\/\/\/\\  
  ...
  for (iter=0; iter<NN; iter++) { //NN is total number of iterations
      if ((OUTPUT >=1)&&((iter % 10)==0)) 
      {
          mexPrintf( "\tIteration %d_W of %d_W\n" , iter , NN );
      ...
      for (ii = 0; ii < n_W; ii++) {
          ...
          if (ni_W>0) {
              ...
              wioffset = entry*T;
              dioffset = di_W*T;
              ep[wioffset+topic]--; //decrement ep count matrix
              dp[dioffset+topic]--; //decrement dp count matrix
              totprob = (double) 0;
              g = 0;
              for (j = 0; j < T; j++) {  //For each topic, calculate the proportional probability
                  for (h = 0; h < ni_W; h++){			  
                      probs[g] = ((double) ep[ entryVec[MaxMatches*wi_W+h]*T + j ] + (double) BETA_W)/( (double) ztot_W[j]+ (double) WBETA)*( (double) dp[ dioffset+ j ] + (double) ALPHA);
                      totprob += probs[g];
                      g++;
                  }
              }
  	       ...
  	       z_W[i] = topic; // assign current word token i to topic j
              ep[entry*T + topic ]++; // and update counts
              dp[dioffset + topic ]++;// increment dp count matrix
              ztot_W[topic]++;
          }
	  }
          
          //mexPrintf( "(2) Working on ii=%d_W i=%d_W wi_W=%d_W di_W=%d_W topic=%d_W ep=%d_W dp=%d_W\n" , ii , i , wi_W , di_W , topic , ep[wi_W+topic*Voc_W] , dp[wi_W+topic*D] );
		  
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~OPINION SAMPLING~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//    if ((OUTPUT==2)&&((iter % 10 ) == 0)) mexPrintf("\tStarting Opinion Sampling; total steps are %d \n", n_O);
	  for (ii = 0; ii<n_O; ii++)
	  {
             ...
             wioffset = wi_O*T;
	      dioffset = di_O*T;
             op[wioffset+topic]--; //decrement op count matrix
 ...
 //mexPrintf("\t wioffset = %d, dioffset = %d, di_W = %d, D= %d \n", wioffset, dioffset, di_W, D);
   ...
   //POPULATE WP MATRIX
  if (OUTPUT==2) mexPrintf("\tPopulating WP matrix\n");
  if (relegate==0) {
      for (i=0; i<n_W; i++)
      {
          wi_W = w_W[ i ];
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs,
                 const mxArray *prhs[])
{
  double *srwp, *srep, *srop, *srdp, *probs, *Z_W, *Z_O, *EntrySeries, *WS, *DS, *OS, *ODS, *L_O, *Z_W_IN, *Z_O_IN, *EntrySeries_IN, *ENTRYVEC, *NumEntriesPerWord; 
  double ALPHA,BETA_W,BETA_O;
  mwIndex *irwp, *irep, *irop, *jcwp, *jcep, *jcop, *irdp, *jcdp;
  int *z_W, *z_O, *d_W, *d_O, *w_W, *w_O, *l_O, *order_W, *order_O, *wp, *ep, *op, *dp, *ztot_W, *ztot_O1, *ztot_O2, *entryVec, *numEntriesPerWord, *entrySeries;
  int MaxMatches, C, Voc_W, Voc_O1, Voc_O2,T,D,NN,SEED,OUTPUT, nzmax, nzmaxwp, nzmaxep, nzmaxop, nzmaxdp, ntokens_W, ntokens_O, nentries_W;
  int i,j,c,n,n_W, n_O,nt,wi_W, wi_O, di_W, di_O, startcond, relegate;
...
//  if (M != nentries_W) mexErrMsgTxt("Number of dictionary entries not the same in ENTRYVEC and ENTRYVEC");
...
/* allocate memory */
  ...
  entryVec = (int *) mxCalloc( nentries_W, sizeof( int ));
 ...
 for (i=0; i<n_O; i++) {
  ...
  D = D + 1;
  C = C + 1;
  ep  = (int *) mxCalloc( T*C , sizeof( int ));
...
//mexPrintf("\tMaxMatches=%d\n", MaxMatches);
  ...
  if (OUTPUT==2) {
      mexPrintf( "Running BOLDA-Multi Gibbs Sampler Version 1.0\n" );
      ...
      mexPrintf( "\tNumber of dict entries C = %d\n"    , C );
      ...
      mexPrintf( "\tw,d_W,z_W,order_W indices combined = %d bytes\n" , 4 * sizeof( int) * ntokens_W );
      mexPrintf( "\tw,d_O,z_O,order_O indices combined = %d bytes\n" , 4 * sizeof( int) * ntokens_O );
      mexPrintf( "\twp (full) matrix - topicwords = %d bytes\n" , sizeof( int ) * Voc_W * T  );
      mexPrintf( "\tep (full) matrix - topicwords = %d bytes\n" , sizeof( int ) * C * T  );
      mexPrintf( "\twp (full) matrix -opinionwords = %d bytes\n" , sizeof( int ) * (Voc_O1 + Voc_O2) * T  );
  ...
  /* run the model */
  GibbsSamplerBOLDA_MultiMatch( ALPHA, BETA_W, BETA_O, Voc_W, Voc_O1, Voc_O2, MaxMatches, C, T, D, NN, OUTPUT, n_W, n_O, z_W, z_O, d_W, d_O, w_W, w_O, ep, wp, dp, op, l_O, entryVec, numEntriesPerWord, entrySeries, ztot_W, ztot_O1, ztot_O2, order_W, order_O, probs, startcond, relegate );
  if (OUTPUT==2) mexPrintf( "Done with sampling! Output time.\n" );
  ...
  *( jcwp + T ) = n;    
  ...
  if (OUTPUT>2) {
      /* convert the full ep matrix into a sparse matrix */
      nzmaxep = 0;
      for (i=0; i<C; i++) {
         for (j=0; j<T; j++) nzmaxep += (int) ( *( ep + j + i*T )) > 0;
      }  
    /*  if (OUTPUT==2) {*/
          mexPrintf( "Constructing sparse output matrix ep\n" );
          mexPrintf( "Number of nonzero entries for EP = %d\n" , nzmaxep );
/*      } */
      // MAKE THE EP SPARSE MATRIX
      plhs[6] = mxCreateSparse( C,T,nzmaxep,mxREAL);
      srep  = mxGetPr(plhs[6]);
      irep = mxGetIr(plhs[6]);
      jcep = mxGetJc(plhs[6]);  
      n = 0;
...
/*  }  */
  ...
  *( jcdp + T ) = n;
      ...
      plhs[1] = mxCreateSparse( 1,1,1,mxREAL);
      srdp  = mxGetPr(plhs[1]);
      irdp = mxGetIr(plhs[1]);
      jcdp = mxGetJc(plhs[1]);
      *(srdp) = 1;
      *(irdp) = 0;
      *( jcdp) = 1;
      plhs[6] = mxCreateSparse( 1,1,1,mxREAL);
      srep  = mxGetPr(plhs[6]);
      irep = mxGetIr(plhs[6]);
      jcep = mxGetJc(plhs[6]);
      *(srep) = 1;
